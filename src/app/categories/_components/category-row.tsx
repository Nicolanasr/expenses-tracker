...
export function CategoryRow({ category }: CategoryRowProps) {
    ...
    return (
        <article className="space-y-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
            <div className="flex items-start justify-between gap-3">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-xs font-medium uppercase text-slate-500">{category.type}</p>
                    </div>
                </div>
                <div className="flex items-center gap-2">
                    <button
                        ...
                        title={isEditing ? "Cancel edit" : "Edit category"}
                    >
                        {isEditing ? "✖️" : "✏️"}
                    </button>
            ...
            <time className="block text-xs text-slate-500" dateTime={category.created_at}>
                Added {formatISODate(category.created_at)}{" "}
            </time>
        </article>
    );
}
